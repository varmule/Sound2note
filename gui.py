...
class RedirectText:
    ...
    def __init__(self, text_widget, max_lines=1000, flush_interval=50):
        ...
        self.flush_interval = flush_interval  # ms
        self.buffer = []
        self.scheduled = False
    def write(self, string):
        self.buffer.append(string)
        if not self.scheduled:
            self.output.after(self.flush_interval, self.flush_buffer)
            self.scheduled = True
    def flush(self):
        pass
    def flush_buffer(self):
        if self.buffer:
            ...
            # Vérifier le nombre de lignes et tronquer si besoin
...
class GUI(object):
    ...
    def setup_ui(self):
        ...
        self.traitement = ttk.Frame(self.notebook)
        ...
        self.freq_entry.grid(row=3, column=1, padx=10, pady=10)
        ...
        self.psnr_label.grid(row=5, column=0, columnspan=2, padx=10, pady=10)

        self.play_original_button = ttk.Button(self.traitement, text="Jouer le signal original", command=self.play_original)
        self.play_original_button.grid(row=6, column=0, padx=10, pady=10)
        ...
        desc_label = ttk.Label(
            content_frame,
            text="Application pour charger, traiter et reconstruire des signaux audio\nà partir de la STFT.",
            font=("Helvetica", 20),
            justify="center"
        )
